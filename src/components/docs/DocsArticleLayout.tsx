  ...
  if (block.type === 'callout') {
    return (
      <div key={index} className="bg-slate-100 dark:bg-slate-950 p-4 rounded-xl border border-slate-200/40 dark:border-slate-800/40">
        <p className="type-technical text-slate-500 dark:text-slate-400">{block.text}</p>
      </div>
    );
  }
      ...
      <div key={index} className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        {block.cards.map(card => (
          <div key={card.title} className="border border-slate-100 dark:border-slate-800/40 p-4 rounded-xl bg-white dark:bg-slate-900 shadow-sm">
            <h4 className="type-card-title-sm text-slate-900 dark:text-white mb-2">{card.title}</h4>
            <p className="type-body-sm text-slate-500 dark:text-slate-400">{card.text}</p>
          </div>
        ))}
      </div>
    );
  ...
  if (block.type === 'table') {
    return (
      <div key={index} className="border border-slate-100 dark:border-slate-800/40 rounded-xl overflow-x-auto mt-4">
        <table className="type-body-sm w-full min-w-[560px] text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800/40">
              {block.headers.map(header => (
                <th key={header} className="type-control p-3 text-slate-900 dark:text-white">{header}</th>
              ))}
            </tr>
          </thead>
            ...
            {block.rows.map((row, rowIndex) => (
              <tr key={rowIndex}>
                {row.map((cell, cellIndex) => (
                  <td key={`${rowIndex}-${cellIndex}`} className={`p-3 ${cellIndex === 0 ? 'font-semibold' : ''}`}>{cell}</td>
                ))}
              </tr>
            ))}
          </tbody>
        ...
        <div className="p-4 sm:p-5">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div className="min-w-0 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="type-technical text-[#5B6CFF] dark:text-[#7C8CFF]">
                  {stage.number} · {stage.phaseLabel}
                </span>
                <span className={`type-label-compact inline-flex rounded-lg px-2 py-1 ${referenceActorStyles[stage.actor]}`}>
                  {stage.actorLabel}
                </span>
              </div>
            ...
            <div className="type-technical shrink-0 rounded-xl bg-[#F7F8FC] px-3 py-2 text-slate-600 dark:bg-[#0B0F1A] dark:text-slate-300">
              {stage.protocol}
            </div>
          </div>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl bg-[#F7F8FC] p-4 dark:bg-[#0B0F1A]">
              <p className="type-label uppercase text-slate-500 dark:text-slate-400">
                {stage.inputLabel}
              </p>
              <div className="mt-3 space-y-2">
                {stage.inputs.map(item => (
                  <p key={item} className="type-body-sm flex items-start gap-2 text-slate-600 dark:text-slate-400">
          ...
          <div className="mt-4 flex items-start gap-3 rounded-2xl border border-[#5B6CFF]/15 bg-[#5B6CFF]/5 p-4">
            ...
            <div>
              <p className="type-label text-slate-900 dark:text-white">{stage.securityLabel}</p>
              <p className="type-body-sm mt-1 text-slate-600 dark:text-slate-300">{stage.security}</p>
            </div>
          </div>
        </div>
      ...
      <div key={index} className="space-y-6 pt-6 border-t border-slate-100 dark:border-slate-800/30">
        {block.items.map(item => (
          <div key={item.version} className="flex flex-col sm:flex-row items-start gap-3 sm:gap-6 pt-6 first:pt-0 first:border-t-0 border-t border-slate-100 dark:border-slate-800/20">
            <span className="type-label px-2.5 py-1 text-[#354CE1] dark:text-[#5F75FF] bg-[#354CE1]/10 border border-[#354CE1]/20 rounded-md shrink-0">
              {item.version}
            </span>
            <div className="space-y-1.5">
              <h4 className="type-card-title text-slate-900 dark:text-white">{item.title}</h4>
              <p className="type-body text-slate-500 dark:text-slate-400">{item.text}</p>
            </div>
          </div>
        ))}
      </div>
    ...
    <>
      <aside className="hidden md:block md:col-span-3 sticky top-32 max-h-[calc(100vh-10rem)] overflow-y-auto scrollbar-none space-y-6 pr-2">
        ...
        <nav aria-label={ui.documentSections} className="space-y-1">
          <h3 className="type-label text-slate-900 dark:text-white uppercase mb-2">{ui.documentSections}</h3>
      ...
      <main aria-label={content.title} className="col-span-1 md:col-span-7 space-y-8 min-h-[60vh] md:pr-4">
        ...
        <div className="border border-slate-100 dark:border-slate-800/40 bg-slate-50/50 dark:bg-slate-900/50 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4 mt-12">
          <div>
            <h4 className="type-card-title-sm text-slate-900 dark:text-white">{ui.helpfulTitle}</h4>
            <p className="type-body-sm text-slate-500 mt-0.5">{ui.helpfulDesc}</p>
          </div>
          {feedbackSubmitted ? (
            <div className="type-control flex items-center gap-2 text-emerald-600 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-150/50 dark:border-emerald-900/40 px-4 py-2 rounded-xl">
              <Check className="w-4 h-4" />
              <span>{ui.thanks}</span>
            </div>
          ) : (
            <div className="flex items-center gap-3">
              <button onClick={() => onFeedback('yes')} className="type-control flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-750/30 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-750 hover:text-emerald-600 transition">
                <ThumbsUp className="w-3.5 h-3.5" />
                <span>{ui.yes}</span>
              </button>
              <button onClick={() => onFeedback('no')} className="type-control flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-750/30 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-750 hover:text-rose-600 transition">
        ...
        <div className="pt-6 border-t border-slate-100 dark:border-slate-850/30 grid grid-cols-1 sm:grid-cols-2 gap-3">
          {previousSection ? (
            <button onClick={() => handleSectionChange(previousSection.id)} className="flex min-h-20 w-full max-w-xs cursor-pointer items-center gap-4 rounded-xl border border-slate-100 bg-white p-4 text-left transition hover:border-[#354CE1]/50 hover:bg-slate-50 dark:border-slate-800/40 dark:bg-slate-900 dark:hover:bg-slate-850">
              ...
              <div>
                <span className="type-label-compact text-slate-400 block uppercase">{ui.previousSection}</span>
                <span className="type-card-title-sm text-slate-900 dark:text-white mt-1 block">{previousSection.title}</span>
              </div>
            </button>
          ...
          {nextSection ? (
            <button onClick={() => handleSectionChange(nextSection.id)} className="flex min-h-20 w-full max-w-xs cursor-pointer items-center justify-between gap-4 rounded-xl border border-slate-100 bg-white p-4 text-left transition hover:border-[#354CE1]/50 hover:bg-slate-50 dark:border-slate-800/40 dark:bg-slate-900 dark:hover:bg-slate-850 sm:justify-self-end">
              <div className="text-left">
                <span className="type-label-compact text-slate-400 block uppercase">{ui.nextSection}</span>
                <span className="type-card-title-sm text-slate-900 dark:text-white mt-1 block">{nextSection.title}</span>
              </div>
      ...
      <aside className="hidden md:block md:col-span-2 sticky top-32 max-h-[calc(100vh-10rem)] overflow-y-auto scrollbar-none pr-2">
        <nav aria-label={ui.sectionTopics} className="space-y-4">
          <h4 className="type-label text-slate-400 uppercase">{ui.sectionTopics}</h4>
          <ul className="type-control space-y-2 border-l border-slate-100 dark:border-slate-850/20">
            {topics.map(topic => (
              <li key={topic.id}>
                <a
                  href={`#${topic.id}`}
