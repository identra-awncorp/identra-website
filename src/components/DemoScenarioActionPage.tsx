...
import { 
  ArrowLeft, Terminal, ShieldCheck, Check, Smartphone, SmartphoneIcon, Sparkles
} from 'lucide-react';

// Import decoupled scenario components
import DemoBankAccount from './demo/DemoBankAccount';
import DemoApplyJob from './demo/DemoApplyJob';
import DemoTicketBooking from './demo/DemoTicketBooking';
import DemoAirlinesHotels from './demo/DemoAirlinesHotels';
import DemoGovernmentServices from './demo/DemoGovernmentServices';
import DemoHealthcare from './demo/DemoHealthcare';
import DemoTicketTransfer from './demo/DemoTicketTransfer';
import DemoSummaryModal from './demo/DemoSummaryModal';
import IdentityFlowGraph from './demo/IdentityFlowGraph';
import { DEMO_SCENARIO_ACTION_TRANSLATIONS } from '../translations/DemoScenarioActionPageTranslations';
...
interface DemoScenarioActionPageProps {
  scenario: Scenario;
  onExit: () => void;
...
export default function DemoScenarioActionPage({ 
  scenario, 
  onExit, 
  playTingTingSound 
}: DemoScenarioActionPageProps) {
  const { language } = useLanguage();
  const t = getLocalizedRecord(DEMO_SCENARIO_ACTION_TRANSLATIONS, language as keyof typeof DEMO_SCENARIO_ACTION_TRANSLATIONS, 'DEMO_SCENARIO_ACTION_TRANSLATIONS');
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-white rounded-[32px] border border-slate-200/80 shadow-xl overflow-hidden relative">
              ...
              <div className="p-6 md:p-8 min-h-[480px] bg-slate-50/50 flex flex-col justify-between">
                ...
                {scenario.id === 'government-services' && (
                  <DemoGovernmentServices
                    currentStepIdx={currentStepIdx}
                    completedSteps={completedSteps}
                    isProcessingAction={isProcessingAction}
                    setIsProcessingAction={setIsProcessingAction}
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 space-y-6 shadow-sm relative overflow-hidden">
              ...
              <div className="grid grid-cols-3 gap-2 bg-slate-50 p-3 rounded-2xl border border-slate-150 text-center text-xs font-mono">
                ...
                <div className="border-x border-slate-200">
                  ...
                  <span className="text-[#354CE1] font-bold text-[10px]">
              ...
              <div className="space-y-4 relative pl-3.5 border-l border-slate-100">
                {scenario.steps.map((st, sIdx) => {
                  ...
                  const subChecks = t.subChecks[scenario.id]?.[sIdx] || [];
            ...
            <div className="bg-slate-900 rounded-[28px] border border-slate-800 p-5 shadow-xl">
              ...
              <div ref={terminalContainerRef} className="font-mono text-[10px] space-y-2 h-52 overflow-y-auto scrollbar-thin text-slate-300 pr-1 select-all leading-relaxed">
                {simulationLogs.map((log, idx) => {
                  ...
                  else if (log.startsWith('[OK]')) color = 'text-[#00D4B2] font-semibold';
