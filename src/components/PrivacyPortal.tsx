...
export default function PrivacyPortal({ onOpenSandbox }: PrivacyPortalProps) {
  ...
  return (
    <section id="privacy" className="section-space-wide bg-[#FAFBFD] overflow-hidden border-b border-slate-100">
        ...
        <div className="text-center stack-heading max-w-2xl mx-auto">
          ...
          <div className="flex justify-center gap-4 pt-2">
            <button
              ...
              className="type-control bg-[#354CE1] hover:bg-[#2539BE] text-white px-5 py-2.5 rounded-full shadow flex items-center gap-1.5 transition cursor-pointer"
            >
              {t.exploreSsi}
        ...
        <div className="max-w-7xl mx-auto bg-white rounded-3xl border border-slate-200/80 p-6 md:p-10 shadow-xl space-y-8 relative">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-100">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="type-card-title text-slate-900">{t.privacyTrustArch}</h3>
                <p className="type-technical text-slate-400 uppercase">{t.decentralizedSsi}</p>
              </div>
            </div>
          ...
          <div className="border-t border-slate-100 pt-6">
            ...
            <div className="bg-slate-50 rounded-xl border border-slate-200/60 overflow-hidden">
              <div className="type-technical grid grid-cols-4 bg-slate-100 p-2.5 text-slate-500 border-b border-slate-200">
                <span>{t.txHash}</span>
              ...
              <div className="divide-y divide-slate-100/80">
                {logs.map((log, i) => (
                  <div key={i} className="grid grid-cols-4 p-3 text-xs text-slate-700 items-center font-mono bg-white">
                    ...
                    <span className="type-caption truncate pr-2 text-slate-500">{log.verifyingParty}</span>
                    <span className="text-right">
                      <span className={`type-technical px-2 py-0.5 rounded-full ${
