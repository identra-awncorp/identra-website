...
for (const filePath of files) {
    ...
    const regex = /const (?:[a-zA-Z0-9]+Translations) = \{\n  en: \{([\s\S]*?)\n  \},\n  vi: \{/s;
    ...
    if (match) {
        ...
        // Fix the match to use the exact variable name that was matched
        const varMatch = content.match(/const ([a-zA-Z0-9]+Translations) = \{\n  en: \{/s);
        
        let newContent = content.replace(regex, `const ${varMatch[1]} = {\n  en: {\n${enBody}\n  },${newEs}${newJa}${newDe}\n  vi: {`);
