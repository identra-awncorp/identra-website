      ...
      <div className="max-w-7xl mx-auto px-6 pt-6 pb-2">
        <div className="flex items-center justify-between">
          <button 
            id="btn-back-to-blog"
            onClick={onBack}
            className="type-control flex items-center gap-2 text-slate-500 hover:text-[#354CE1] transition group"
          >
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {copyStatus !== 'idle' ? (
                <span className={`type-caption absolute -top-9 left-1/2 -translate-x-1/2 text-white py-1 px-2 rounded-md whitespace-nowrap ${copyStatus === 'success' ? 'bg-slate-900' : 'bg-rose-600'}`}>
            ...
            <button
              ...
              className="type-control bg-[#354CE1] hover:bg-[#2539BE] text-white px-4 py-2 rounded-full shadow-sm transition"
        ...
        <div className="bg-gradient-to-tr from-[#354CE1] via-[#5F3CF3] to-[#00D4B2] text-white rounded-[32px] p-8 md:p-12 lg:p-14 relative overflow-hidden shadow-xl shadow-indigo-950/5">
          ...
          <div className="relative z-10">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mb-6">
              <span className="type-label bg-white/20 backdrop-blur-md border border-white/25 text-yellow-300 px-3 py-1 rounded-full uppercase">{t.copy.blogGuide}</span>
              <div className="type-body-sm flex items-center gap-1.5 text-indigo-100">
            ...
            <h1 className="type-document-title measure-display text-balance max-w-4xl mb-6">{currentPost.title}</h1>
            
            <p className="type-lead measure-lead text-slate-200 max-w-3xl mb-8">{currentPost.description}</p>

            <div className="flex items-center gap-3 border-t border-white/10 pt-6 mt-8">
              <div className="type-control w-10 h-10 rounded-full bg-gradient-to-tr from-[#354CE1] to-[#BE185D] flex items-center justify-center text-white border-2 border-white/20 shadow-md">
                BC
              </div>
              <div>
                <p className="type-body-sm font-bold text-white">Brandon Chen</p>
                <p className="type-body-sm text-slate-300">{t.copy.productMarketingIdentra}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="type-body lg:col-span-8 space-y-8 text-slate-700">
            ...
            <div ref={secHowItWorks} id="how-sentinel-works" className="pt-2 scroll-mt-24 space-y-4">
              ...
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-white p-5 rounded-2xl border border-slate-150/80 shadow-xs">
                {dataT.signals.map((item, index) => (
                  <li key={index} className="type-body-sm flex items-center gap-2.5 font-medium text-slate-700">
              ...
              <div className="space-y-4">
                {dataT.capabilities.map((item, index) => (
                  <div key={index} className="bg-white border border-slate-100 p-5 rounded-2xl shadow-xs space-y-1.5">
                    <h4 className="type-card-title text-slate-900 flex items-center gap-2">
              ...
              <div className="bg-linear-to-b from-slate-50 to-indigo-50/20 border border-slate-200/60 rounded-3xl p-6 md:p-8 my-8">
                <div className="flex items-center justify-between border-b border-slate-200/80 pb-4 mb-6">
                  <div>
                    <h4 className="type-card-title text-slate-900">{t.copy.interactiveShowcase}</h4>
                    <p className="type-body-sm text-slate-500">{t.copy.realTimeRiskSignallingThroughoutSessionLifecycle}</p>
                  </div>
                  <span className="type-label bg-emerald-50 text-emerald-700 uppercase px-2 py-0.5 rounded-full border border-emerald-100">{t.copy.liveSimulator}</span>
                </div>
                ...
                <div className="space-y-6 max-w-md mx-auto">
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="relative flex-1 bg-white border border-slate-200/80 p-4 rounded-2xl shadow-xs">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <div>
                          <span className="type-label text-slate-400 uppercase">{t.copy.action}</span>
                          <h5 className="text-xs font-bold text-slate-900 leading-none mt-0.5">{t.copy.idVerification}</h5>
                        </div>
                      </div>
                      
                      <p className="type-technical text-slate-500 bg-slate-50 p-2 rounded-lg border border-slate-100">{t.copy.uniqueDeviceToken}<span className="font-bold text-indigo-600">dev_Z8Nqm2LPr7xK</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="relative flex-1 bg-white border border-slate-200/80 p-4 rounded-2xl shadow-xs">
                      ...
                      <p className="type-technical text-slate-500 bg-slate-50 p-2 rounded-lg border border-slate-100">{t.copy.uniqueDeviceToken}<span className="font-bold text-indigo-600">dev_M4xJq8NzT2vL</span>
                      </p>
                      ...
                      <div className="mt-3 bg-amber-50 border border-amber-200 p-3 rounded-xl flex gap-2.5 items-start">
                        ...
                        <div>
                          <h6 className="type-label text-amber-800 uppercase">{t.copy.warningFlag}</h6>
                ...
                {dataT.fiveWays.map((item, index) => (
                  <div key={index} className="bg-white rounded-2xl border border-slate-100 p-6 shadow-xs space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="type-card-title text-slate-950">
                        {item.title}
                      </h3>
                    </div>
            ...
            <div className="bg-white border border-slate-150/80 rounded-3xl p-6 md:p-8 mt-12 space-y-6">
              <div>
                <h3 className="type-card-title text-slate-950">{t.copy.faqs}</h3>
                <p className="type-body-sm text-slate-500">{t.copy.commonQuestionsRegardingSentinelIntegrationAndTechnology}</p>
              </div>
            ...
            <div className="bg-[#FAFBFD] border border-slate-200/60 rounded-2xl p-6 flex flex-col sm:flex-row gap-5 items-center sm:items-start text-center sm:text-left">
              <div className="type-card-title w-16 h-16 rounded-full bg-gradient-to-tr from-[#354CE1] to-[#BE185D] flex items-center justify-center text-white border-4 border-white shadow-md shrink-0">
                BC
              </div>
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-36 space-y-8">
              ...
              <nav className="flex flex-col gap-1">
                {tableOfContents.map((item) => (
                  <button
                    ...
                    className={`type-control text-left px-3 py-2 rounded-lg transition-all ${
            ...
            <div className="bg-gradient-to-b from-indigo-900 to-[#12183A] text-white rounded-2xl p-6 shadow-md space-y-4">
              ...
              <div className="space-y-1">
                <h4 className="type-card-title">{t.copy.needAContinuousRiskSignalsStrategy}</h4>
                <p className="type-body-sm text-slate-300">{t.copy.configureSentinelsPassiveTelemetryToAnalyzeBrowsersProxy}</p>
              </div>
              <button
                onClick={onOpenSandbox}
                className="type-control w-full bg-[#354CE1] hover:bg-[#2539BE] text-white py-2.5 rounded-xl transition shadow-sm flex items-center justify-center gap-1.5"
              >
            ...
            {dataT.continueCards.map((card, idx) => (
              ...
              >
                <div className="space-y-3">
                  <div className="type-label flex items-center gap-2 text-slate-400 uppercase">
                    <span>{card.type}</span>
                    <span aria-hidden="true">&middot;</span>
                    <span>{card.time}</span>
                  </div>
                  <h4 className="type-card-title text-slate-900 group-hover:text-[#354CE1] line-clamp-2 transition-colors">
                    {card.title}
                  </h4>
                  <p className="type-body-sm text-slate-500 line-clamp-3">
                    {card.desc}
                  </p>
                </div>

                <div className="type-control flex items-center gap-1.5 text-[#354CE1] mt-5">
